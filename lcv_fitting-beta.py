...
def raw_lcv(lcv_data, period, t0, lcv_name, band='V'):

    # read in GLOESS fit
    dt = np.dtype([('filter', 'S2'), ('phase', float), ('mag', float)])
    gloess_fit = np.loadtxt(lcv_name+'.fit', dtype=dt)

    filt_select = lcv_data['filter'] == band
    gloess_select = gloess_fit['filter'] == band

    t = lcv_data['mjd'][filt_select]
    t_min = np.min(t)
    t_max = np.max(t)
    ...
    def update(val):
        pos = spos.val
        ax[1].axis([pos,pos+5,np.max(y)+0.2,np.min(y)-0.2])
        ax[0].fill_between([pos, pos+5], v,v, alpha=0.3, color='r')
